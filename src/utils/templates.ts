...
export function readTemplates(fromDirectory: string[], toDirectory: string, excludedDirs: string[] = []): CopyDescriptor[] {
  ...
  return files.reduce((prev, curr) => {
    if (curr.isDirectory() && !excludedDirs.includes(curr.name)) {
